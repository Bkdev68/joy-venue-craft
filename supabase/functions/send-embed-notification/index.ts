...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    const customerHtml = `<div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;"><h1 style="color: #D4AF37;">Vielen Dank für Ihre Anfrage!</h1><p>Hallo ${safeValue(data.customerName)},</p><p>wir haben Ihre Anfrage erhalten und werden uns innerhalb von 24 Stunden bei Ihnen melden.</p><h2 style="color: #333; margin-top: 20px;">Ihre Anfrage im Überblick</h2><div style="background-color: #f9f9f9; padding: 20px; border-radius: 10px;"><p><strong>Datum:</strong> ${formattedDate}</p>${data.eventTime ? `<p><strong>Uhrzeit:</strong> ${safeValue(data.eventTime)} Uhr</p>` : ''}${data.duration ? `<p><strong>Dauer:</strong> ${data.duration} Stunden</p>` : ''}<p><strong>Event-Art:</strong> ${safeValue(data.eventType)}</p><p><strong>Gewünschte Leistung:</strong> ${safeValue(data.serviceName)}</p>${data.venue ? `<p><strong>Veranstaltungsort:</strong> ${safeValue(data.venue)}</p>` : ''}</div><p style="margin-top: 20px;">Bei Fragen können Sie uns jederzeit kontaktieren.</p><p>Mit freundlichen Grüßen,<br><strong>Ihr PixelPalast Team</strong></p><hr style="border: none; border-top: 1px solid #ddd; margin: 30px 0;"><p style="color: #999; font-size: 12px;">PixelPalast - Photo Booth & 360° Video Booth<br><a href="https://pixelpalast.at" style="color: #D4AF37;">www.pixelpalast.at</a></p></div>`;
